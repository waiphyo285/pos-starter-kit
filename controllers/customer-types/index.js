...
const CustomerType = require('@models/mongodb/schemas/customer-type')
...
const service = services.customer_type
...
controller.listData = async (params) => {
    ...
    const recordsTotal = await CustomerType.countDocuments(filter)

    return CustomerType.find(filter)
        .or({
            $or: w_regx,
        })
        .sort(sort)
...
controller.findDataById = (id) => {
    return CustomerType.findById(id).lean().then(serialize)
}

controller.findDataBy = async (params) => {
    const { filter } = await getPaginationQuery(params)
    return CustomerType.find(filter).lean().then(serialize)
}

controller.addData = async (dataObj) => {
    return CustomerType.create(dataObj).then(serialize)
}

controller.updateData = (id, dataObj) => {
    return CustomerType.findByIdAndUpdate(id, dataObj).lean().then(serialize)
}
...
controller.deleteData = async (id) => {
    ...
    // return CustomerType.findByIdAndDelete(id).lean().then(serialize)
    return CustomerType.deleteById(id).then(serialize) // softdelete
}
